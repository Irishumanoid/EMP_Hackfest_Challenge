    ...
    return (
        <div className="flex h-full bg-gradient-to-b from-light-200 to-light-400 shadow-lg flex-shrink-0" style={{boxShadow: "0px 0px 10px -3px rgba(0,0,0,0.5)"}}>
            ...
            <div className={`flex-shrink-0 w-screen h-full ${sidebarOpen ? 'sm:max-w-xs max-w-screen-sm' : 'max-w-0'} transition-all overflow-hidden`}>
                <div className="w-screen sm:max-w-xs max-w-screen-sm h-screen relative">
                    <div className="w-full py-4 text-center text-xl font-bold ">Product Name</div>
                    <div className="flex flex-col px-4 h-full overflow-x-scroll custom-scrollbar">
                        {props.posts.map((post, index)=>{
                            return (
                                <div className="p-4 rounded-lg shadow-md bg-gradient-to-b from-light-300 to-light-400 mb-5">
